  ...
  return (
    <div className="h-full w-full flex flex-col bg-slate-50 dark:bg-slate-950">
      ...
      <div className="flex flex-1 min-h-0">
        <aside className="w-64 border-r border-slate-200 dark:border-slate-800 p-3 overflow-y-auto">
        ...
        <main className="flex-1 min-w-0 flex flex-col">
          {!active ? <div className="m-auto text-slate-500">Create a project to start.</div> : <>
            <div className="flex-1 min-h-0 flex">
              <div className="w-56 border-r border-slate-200 dark:border-slate-800 p-3 overflow-y-auto">
            ...
            <div className="border-t border-slate-200 dark:border-slate-800 p-3 space-y-3">
              ...
              <div className="flex gap-2"><textarea value={prompt} onChange={e => setPrompt(e.target.value)} placeholder="Tell Lumen what to build or change in this project..." className="flex-1 min-h-16 max-h-32 resize-y rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-3 text-sm" /><button onClick={() => void runTask()} disabled={!prompt.trim()} className="self-end rounded-xl bg-brand-600 text-white px-4 py-2 text-sm disabled:opacity-50">Build</button></div>
